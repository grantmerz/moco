...
import custom_models.phot_lat_map as custom_map
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    photinfo = pd.read_csv(args.data)
    photinfo = photinfo.drop(columns=['COADD_OBJECT_ID'])
    photinfo = photinfo.values
    encoded = np.load(args.labels)

    
    
    
    photscaler = StandardScaler()
    photinfo = photscaler.fit_transform(photinfo)

    encscaler = StandardScaler()
    encoded = encscaler.fit_transform(encoded)
